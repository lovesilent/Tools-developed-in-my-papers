...
public class ReadFiles {
	...
	public ArrayList<FacebookNode> getEgos(ArrayList<FacebookNode> allNodeSimilarityList, int egoNetworkNodeNumber) throws FileNotFoundException{
		UtilityFunctions utilityFunctions = new UtilityFunctions();
		File file = new File("Facebook_Data/"+egoNetworkNodeNumber+".egos");
		String line = "";
		FacebookNode fbNode = null;
		Scanner scan = new Scanner(new FileReader(file));
		// Read the file line by line and store the nodes in the ego of a FacebookNode
		while(scan.hasNextLine()){
			line = scan.nextLine();
			String[] numbers = line.split(" ");
			fbNode = utilityFunctions.getFBNode(allNodeSimilarityList, ("N"+numbers[0]));
